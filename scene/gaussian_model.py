...
class GaussianModel:
    ...
    def setup_functions(self): #设置激活函数/变换函数
        #构建协方差矩阵
    ...
    def __init__(self, sh_degree : int):
        ...
        self.spatial_lr_scale = 0 #学习速率scale
        self.setup_functions() #设置激活/变换函数
    ...
    #从初始点云pcd创建3dgs的初始状态
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float): #从初始点云pcd创建3dgs的初始状态
    ...
    def reset_opacity(self):
        opacities_new = inverse_sigmoid(torch.min(self.get_opacity, torch.ones_like(self.get_opacity)*0.01)) #将透明度重置为一个很小的值。
        optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity")
        self._opacity = optimizable_tensors["opacity"]

    def load_ply(self, path): #从PLY文件中加载各种数据
